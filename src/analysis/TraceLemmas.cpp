...
#pragma mark - Lemmas for two traces
                                                      ...
                                                      std::vector<std::shared_ptr<const logic::Formula>>& lemmas)
    {
        ...
        // add lemma for each intVar
        // Lemma: forall ((x : Int)). (v l(zero) <= x & x < v l(n) & forall ((it : Nat)).(v l(s(it)) = (v l(it) + 1))
        ...
        for (const auto& v : locationToActiveVars.at(locationName))
        {
            if (!v->isConstant)
            {
                if (!v->isArray)
                {
                    ...
                    // Part 1.3: forall it.(v l(s(it)) = (v l(it) +1))
                    ...
                    auto vequal = logic::Formulas::equality(vsit,vitpp);

                    conjunctsLHS.push_back(logic::Formulas::universal({iSymbol},vequal));
                    ...
                    auto bareLemma = logic::Formulas::universal({xSym},combined,label);
